...
import { getGridBandInfo } from "@/logic/gridPhysicsEngine";
...
export function GridStabilityBadge({ stability }: Props) {
  const { label, color } = getGridBandInfo(stability);

  return (
    <View style={s.row}>
      <MaterialCommunityIcons name="transmission-tower" size={11} color={color} />
      <Text style={s.section}>RÉSEAU ÉLECTRIQUE</Text>
